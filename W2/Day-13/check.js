...
function getNameOfDriverWithoutDL(fp) {
    const result = fp.reduce((acc,curr) => {
                if(!curr.hasDrivingLicense && curr.likes){
                    for(let i in curr.likes){
                        if(curr.likes[i] === 'driving'){
                            curr.push(acc.firstName)
                        } 
                    }
                }
                },[]);
